...
public class CsvExportUtil {
    ...
    public static void doExport(List<Map<String, Object>> dataList, String titles, String keys, OutputStream os) throws Exception {
        ...
        for(int i = 0 ; i < titleArr.length ; i++){
            if(i != titleArr.length - 1) buf.append(titleArr[i]).append(CSV_COLUMN_SEPARATOR);
            else buf.append(titleArr[i]);
        }
        ...
        if (CollectionUtils.isNotEmpty(dataList)) {
            for (Map<String, Object> data : dataList) {
                for(int i = 0 ; i < keyArr.length ; i++){
                    if(i != keyArr.length - 1) buf.append(data.get(keyArr[i])).append(CSV_COLUMN_SEPARATOR);
                    else buf.append(data.get(keyArr[i]));
                }
